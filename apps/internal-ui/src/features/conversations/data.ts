...
export function statusFor(mode: ControlMode, assignedOperatorName?: string): StatusInfo {
  ...
  if (mode === "human") return { label: "Ведёте вы", color: "var(--primary-text)", bg: "var(--primary-bg)", border: "var(--primary-border)", dot: "var(--primary)" };
  if (mode === "assigned") return { label: assignedOperatorName ? `Ведёт ${assignedOperatorName}` : "Ведёт другой сотрудник", color: "var(--n-3)", bg: "var(--n-10)", border: "var(--n-6)", dot: "var(--n-4)" };
  if (mode === "closed") return { label: "Диалог закрыт", color: "var(--n-3)", bg: "var(--n-9)", border: "var(--n-6)", dot: "var(--n-5)" };
  return { label: "Ждёт человека", color: "var(--warning-text)", bg: "var(--warning-bg)", border: "var(--warning-border)", dot: "var(--warning)" };
}
