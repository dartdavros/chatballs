  ...
  return (
    <Modal open={open} onCancel={onClose} footer={null} closable={false} centered width="auto" className="image-lightbox" rootClassName="image-lightbox-root" destroyOnHidden>
      <div className="image-lightbox-bar">
        ...
        <a className="image-lightbox-action" href={downloadUrl} download={name} title="Скачать"><Icon name="download" size={16} /><span>Скачать</span></a>
        <button className="image-lightbox-action" type="button" aria-label="Закрыть" title="Закрыть · Esc" onClick={onClose}><Icon name="xCircle" size={18} /></button>
      </div>
