      ...
      <div className="sales-client-overview-rail">
        {current && (
          <section className="sales-client-section-card">
            ...
            <Button className="sales-client-outline-action" variant="secondary" onClick={() => openConversation(current.id)}>Открыть диалог</Button>
          </section>
        )}
        {current?.assignee && (
          <section className="sales-client-section-card">
            ...
            <div className="sales-client-assignee">
              ...
              <span><strong>{current.assignee}</strong><small>по текущему диалогу</small></span>
            </div>
          </section>
        )}
        {noteDialog && (
...
function ActivityList({ activity }: { activity: ClientDetailVm["activity"] }) {
  return (
    <section className="sales-client-section-card">
      ...
      {activity.length === 0 ? (
        <div className="sales-client-related-empty">Нет событий</div>
      ) : (
