  ...
  return (
    <header className="hub-topbar">
      <div className="breadcrumbs">
        <button type="button" onClick={() => setRoute("chat")}>{user.organizationName}</button>
        <i>/</i>
        ...
        {route === "supportPortalDetail" && <><button type="button" onClick={() => setRoute("supportPortals")}>{routes.supportPortals}</button><i>/</i><strong>{routes[route]}</strong></>}
        ...
        {route !== "employeeDetail" && route !== "agentDetail" && route !== "supportPortalDetail" && route !== "salesClientDetail" && !isAi && !isAiDetail && <strong>{routes[route]}</strong>}
      </div>
