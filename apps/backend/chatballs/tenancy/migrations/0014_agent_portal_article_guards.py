...
def install(apps, schema_editor):
    schema_editor.execute(
        f"""
        ALTER TABLE {LINK_TABLE} OWNER TO chatballs_schema;
        ...
        GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE {LINK_TABLE} TO chatballs_runtime_app;
        GRANT ALL ON TABLE {LINK_TABLE} TO chatballs_schema;
        DROP POLICY IF EXISTS chatballs_tenant_isolation ON {LINK_TABLE};
        CREATE POLICY chatballs_tenant_isolation ON {LINK_TABLE}
            FOR ALL TO chatballs_runtime_app
            USING ({LINK_POLICY}) WITH CHECK ({LINK_POLICY});
        DROP POLICY IF EXISTS chatballs_schema_access ON {LINK_TABLE};
        CREATE POLICY chatballs_schema_access ON {LINK_TABLE}
