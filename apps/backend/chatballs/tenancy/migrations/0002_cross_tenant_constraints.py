...
def add_constraints(apps, schema_editor):
    schema_editor.execute("CREATE SCHEMA IF NOT EXISTS chatballs")
    schema_editor.execute(
        """
        CREATE OR REPLACE FUNCTION chatballs.enforce_tenant_fk() RETURNS trigger
        LANGUAGE plpgsql AS $$
        ...
        CREATE OR REPLACE FUNCTION chatballs.enforce_tenant_user() RETURNS trigger
        LANGUAGE plpgsql AS $$
        ...
        CREATE OR REPLACE FUNCTION chatballs.enforce_tenant_pair() RETURNS trigger
        LANGUAGE plpgsql AS $$
    ...
    with schema_editor.connection.cursor() as cursor:
        ...
        for index, (table, user_column) in enumerate(TENANT_USER_FIELDS):
            ...
            schema_editor.execute(
                ...
                "DEFERRABLE INITIALLY IMMEDIATE FOR EACH ROW EXECUTE FUNCTION "
                f"chatballs.enforce_tenant_user('{user_column}')"
            )
        for index, (table, left_table, left_column, right_table, right_column) in enumerate(TENANT_PAIRS):
            ...
            schema_editor.execute(
                ...
                "DEFERRABLE INITIALLY IMMEDIATE FOR EACH ROW EXECUTE FUNCTION "
                f"chatballs.enforce_tenant_pair('{left_table}', '{left_column}', "
...
def remove_constraints(apps, schema_editor):
    ...
    schema_editor.execute("DROP FUNCTION IF EXISTS chatballs.enforce_tenant_pair()")
    schema_editor.execute("DROP FUNCTION IF EXISTS chatballs.enforce_tenant_user()")
    schema_editor.execute("DROP FUNCTION IF EXISTS chatballs.enforce_tenant_fk()")
