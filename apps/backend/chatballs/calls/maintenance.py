...
from chatballs.calls.models import CallEndedBy, CallSession, CallStatus
...
def expire_stale_calls(context) -> int:
    ...
    connect_deadline = now - timedelta(seconds=settings.CHATBALLS_CALL_CONNECT_GRACE_SECONDS)
    ...
    reconnect_deadline = now - timedelta(seconds=settings.CHATBALLS_CALL_RECONNECT_GRACE_SECONDS)
    dropped = (
