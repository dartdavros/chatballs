...
def _delete_record(record: DemoRecord, demo_keys: set[tuple[int, str]]) -> bool:
    ...
    Защищающие объекты (звонок ↔ диалог, статья ↔ ревизия) удаляются первыми,
    затем повтор — они не могут жить без демо-объекта. Обнуление ссылок — последний шанс для
    циклов; если оно ломает check-constraint, объект просто откладывается."""
    ...
    for attempt in range(3):
        try:
            ...
            # Защищающие объекты (звонок по демо-диалогу, ревизия демо-статьи)
            # не могут существовать без демо-объекта — удаляются вместе с ним,
            # даже если их создал тестировавший человек, а не сид.
            protectors = list(error.protected_objects)
            foreign = [
                obj for obj in protectors
                if (ContentType.objects.get_for_model(type(obj), for_concrete_model=True).id, str(obj.pk)) not in demo_keys
            ]
            if foreign and attempt == 0:
                logger.warning(
                    "Demo removal: %s %s is referenced by %d non-demo object(s) created on top of the demo — removing them too",
                    model.__name__, record.object_id, len(foreign),
                )
            if attempt == 0:
                for obj in protectors:
                    try:
                        with transaction.atomic():
                            _delete_files(obj)
                            obj.delete()
                ...
                try:
                    with transaction.atomic():
                        if not _detach_demo_references(instance, demo_keys):
                            return False
