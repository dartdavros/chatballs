...
def _attachments(message) -> tuple[InboundFile, ...]:
    """Вложения письма → файлы диалога; слишком большие пропускаются."""
    ...
    for part in message.iter_attachments():
        ...
        if not isinstance(content, bytes) or not content or len(content) > MAX_ATTACHMENT_BYTES:
            continue
        name = safe_filename(part.get_filename() or "", "attachment")
        mime = str(part.get_content_type() or "") or guess_content_type(name)
        files.append(InboundFile(name=name, content_type=mime, size=len(content), content=content, is_image=mime.startswith("image/")))
    return tuple(files)
...
def send_text(integration, *, chat_id: str, user_id: str, text: str, attachments: list[tuple[bytes, str, str]] | None = None) -> bool:
    """Ответ письмом; attachments — [(content, filename, content_type)]."""
    ...
    for content, filename, content_type in attachments or []:
        ...
        outgoing.add_attachment(content, maintype=maintype or "application", subtype=subtype or "octet-stream", filename=filename)
