...
from hub_platform.integrations.models import IntegrationKind, IntegrationProvider, IntegrationStatus
...
class EmailConfigTests(TestCase):
    def setUp(self) -> None:
        ...
        self.context = system_tenant_context(Organization.objects.get(slug="edevs"))
    ...
    def test_serializer_exposes_email_config(self) -> None:
        ...
        self.assertEqual(config["email"], "support@edevs.tech")
...
class EmailCheckTests(TestCase):
    ...
    def test_both_sides_ok(self) -> None:
        ...
        self.assertTrue(ok)
        self.assertEqual(detail, "Email: support@edevs.tech")
        imap_cls.return_value.login.assert_called_once_with("support@edevs.tech", "pw")
        smtp_cls.return_value.login.assert_called_once_with("support@edevs.tech", "pw")
...
class EmailTestIntegrationDispatchTests(TestCase):
    ...
    def test_email_check_updates_status(self) -> None:
        from hub_platform.integrations.services import test_integration as run_integration_test

        integration = create_integration(
            context=self.context,
            data=IntegrationInput(provider=IntegrationProvider.EMAIL, name="Почта", secret="pw", config=dict(EMAIL_INPUT)),
        )
        with mock.patch.object(checks, "check_email", return_value=(True, "Email: support@edevs.tech", {})) as check:
