...
def turn_credentials(*, label: str = "hub", now: int | None = None) -> tuple[str, str]:
    ...
    его `static-auth-secret`. Секрет (`HUB_CALL_TURN_SECRET`) общий с сервисом
    coturn и наружу не отдаётся — клиент получает только производные credentials.
    """
    moment = int(time.time()) if now is None else int(now)
    expiry = moment + settings.HUB_CALL_TURN_TTL_SECONDS
    username = f"{expiry}:{label}"
    digest = hmac.new(
        settings.HUB_CALL_TURN_SECRET.encode("utf-8"),
