...
def _custoai_provider() -> CustoAIProvider:
    if not settings.HUB_CUSTOAI_API_KEY:
        raise ImproperlyConfigured("HUB_CUSTOAI_API_KEY is required for CustoAI")
    if not settings.HUB_CUSTOAI_MODEL:
        raise ImproperlyConfigured("HUB_CUSTOAI_MODEL is required for CustoAI")
    return CustoAIProvider(
        api_key=settings.HUB_CUSTOAI_API_KEY,
        base_url=settings.HUB_CUSTOAI_BASE_URL,
        model=settings.HUB_CUSTOAI_MODEL,
...
def get_provider(*, channel=None) -> LLMProvider:
    ...
    """
    if settings.HUB_AI_PROVIDER == "test":
    ...
    raise ImproperlyConfigured(f"Unknown AI credential mode: {mode}")
