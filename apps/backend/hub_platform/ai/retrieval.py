...
from hub_platform.ai.models import KnowledgeFragment, ProductAIRelease


def _release_fragments(release: ProductAIRelease):
...
def lexical_search(release: ProductAIRelease, query: str, *, limit: int = 5) -> list[KnowledgeFragment]:
    if not query.strip():
        return []
...
def semantic_search(release: ProductAIRelease, query_vector: list[float], *, limit: int = 5) -> list[KnowledgeFragment]:
    return list(
        _release_fragments(release)
...
class KnowledgeRetriever:
    ...
    def retrieve(self, *, release: ProductAIRelease, query: str, limit: int = 5) -> list[KnowledgeFragment]:
