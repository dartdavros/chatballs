...
def close_stale_conversations(days: int = AUTOCLOSE_DAYS) -> int:
    cutoff = timezone.now() - timedelta(days=days)
    closed = Conversation.objects.filter(lifecycle=LifecycleState.OPEN, last_activity_at__lte=cutoff).update(
