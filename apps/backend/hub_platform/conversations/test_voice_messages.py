...
class VoiceIngestTests(VoiceTestCase):
    def test_inbound_voice_is_stored_and_queued_for_operator(self) -> None:
        with mock.patch(
            "hub_platform.conversations.ingest.transports.download_voice",
...
class VoiceApiTests(VoiceTestCase):
    ...
    def test_transcribe_uses_provider_and_persists(self) -> None:
        ...
        with mock.patch(
            "hub_platform.ai.provider.local.LocalProvider.transcribe"
        ) as transcribe:
    ...
    def test_operator_sends_voice_to_telegram(self) -> None:
        ...
        with mock.patch(
            "hub_platform.conversations.transports.send_voice", return_value=True
        ) as send:
            response = self.client.post(
