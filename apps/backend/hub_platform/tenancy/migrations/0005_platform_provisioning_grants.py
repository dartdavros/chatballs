...
#
# Provisioning runs on the platform connection (custocrm_runtime_platform) and must
...
# identity_employeeprofile grant DML to custocrm_runtime_app only. This migration
...
# 0003 for custocrm_runtime_app. Here we grant the platform role DML and add a
...
def apply_grants(apps, schema_editor):
    ...
    schema_editor.execute(
        "GRANT INSERT, UPDATE ON identity_organization TO custocrm_runtime_platform"
    )

    # 2. For each provisioning tenant table: grant DML to the platform role and add
    #    a tenant-isolation policy mirroring custocrm_subscription_tenant.
    for table in PROVISIONING_TENANT_TABLES:
        schema_editor.execute(
            f"GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE {table} TO custocrm_runtime_platform"
        )
    ...
    schema_editor.execute(
        """
        CREATE OR REPLACE VIEW custocrm.organization_directory
        WITH (security_barrier = true) AS
            SELECT id AS organization_id, public_id, status, slug
            FROM identity_organization
        """
    )
    schema_editor.execute("ALTER VIEW custocrm.organization_directory OWNER TO custocrm_schema")
    schema_editor.execute("REVOKE ALL ON custocrm.organization_directory FROM PUBLIC")
    schema_editor.execute(
        "GRANT SELECT ON custocrm.organization_directory TO custocrm_runtime_platform"
    )


def revert_grants(apps, schema_editor):
    schema_editor.execute("DROP VIEW IF EXISTS custocrm.organization_directory")
    for table in PROVISIONING_TENANT_TABLES:
        schema_editor.execute(f"DROP POLICY IF EXISTS {PLATFORM_POLICY} ON {table}")
        schema_editor.execute(
            f"REVOKE SELECT, INSERT, UPDATE, DELETE ON TABLE {table} FROM custocrm_runtime_platform"
        )
    schema_editor.execute(
        "REVOKE INSERT, UPDATE ON identity_organization FROM custocrm_runtime_platform"
    )
